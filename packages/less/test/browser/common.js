...
ieFormat = function (text) {
  ...
  const transformedText = styleNode.styleSheet ? styleNode.styleSheet.cssText : styleNode.innerText
...
testSheet = function (sheet) {
  it(sheet.id + ' should match the expected output', function (done) {
    const lessOutputId = sheet.id.replace('original-', '')
    const expectedOutputId = 'expected-' + lessOutputId
    let lessOutputObj
    let lessOutput
    const expectedOutputHref = document.getElementById(expectedOutputId).href
    const expectedOutput = loadFile(expectedOutputHref)
...
testErrorSheet = function (sheet) {
  it(sheet.id + ' should match an error', function (done) {
    const lessHref = sheet.href
    const id = 'less-error-message:' + extractId(lessHref)
    const errorHref = lessHref.replace(/.less$/, '.txt')
    const errorFile = loadFile(errorHref)
    let actualErrorElement
    let actualErrorMsg
    ...
    waitFor(function () {
      ...
      const innerText = (actualErrorElement.innerHTML
...
testErrorSheetConsole = function (sheet) {
  it(sheet.id + ' should match an error', function (done) {
    const lessHref = sheet.href
    const id = sheet.id.replace(/^original-less:/, 'less-error-message:')
    const errorHref = lessHref.replace(/.less$/, '.txt')
    const errorFile = loadFile(errorHref)
    const actualErrorElement = document.getElementById(id)
    const actualErrorMsg = logMessages[logMessages.length - 1]
      .replace(/\nStack Trace\n[\s\S]*/, '')
