...
const logger = require('../lib/less/logger').default

const isVerbose = process.env.npm_config_loglevel !== 'concise'
logger.addListener({
  info (msg) {
    if (isVerbose) {
...
module.exports = function () {
  ...
  const fs = require('fs')
  const clone = require('copy-anything').copy

  const less = require('../')

  const stylize = require('../lib/less-node/lessc-helper').stylize
  ...
  let failedTests = 0
  let passedTests = 0
  const finishTimer = setInterval(endTest, 500)
  ...
  function testSourcemap (name, err, compiledLess, doReplacements, sourcemap, baseFolder) {
    ...
    const expectedSourceMapURL = name + '.css.map'
    const sourceMappingPrefix = '/*# sourceMappingURL='
    const sourceMappingSuffix = ' */'
    const expectedCSSAppendage = sourceMappingPrefix + expectedSourceMapURL + sourceMappingSuffix
    if (!compiledLess.endsWith(expectedCSSAppendage)) {
      ...
      const indexOfSourceMappingPrefix = compiledLess.indexOf(sourceMappingPrefix)
      if (indexOfSourceMappingPrefix === -1) {
        fail('ERROR: sourceMappingURL was not found in ' + baseFolder + '/' + name + '.css.')
        return
      }

      const startOfSourceMappingValue = indexOfSourceMappingPrefix + sourceMappingPrefix.length
      const indexOfNextSpace = compiledLess.indexOf(' ', startOfSourceMappingValue)
      const actualSourceMapURL = compiledLess.substring(startOfSourceMappingValue, indexOfNextSpace === -1 ? compiledLess.length : indexOfNextSpace)
  ...
  function testSourcemapWithoutUrlAnnotation (name, err, compiledLess, doReplacements, sourcemap, baseFolder) {
    ...
    const sourceMapRegExp = /\/\*# sourceMappingURL=.+\.css\.map \*\/$/
  ...
  function globalReplacements (input, directory, filename) {
    const path = require('path')
    const p = filename ? path.join(path.dirname(filename), '/') : directory
    const pathimport = path.join(directory + 'import/')
    const pathesc = p.replace(/[.:/\\]/g, function (a) { return '\\' + (a == '\\' ? '\/' : a) })
    const pathimportesc = pathimport.replace(/[.:/\\]/g, function (a) { return '\\' + (a == '\\' ? '\/' : a) })
  ...
  function runTestSetInternal (baseFolder, opts, foldername, verifyFunction, nameModifier, doReplacements, getFilename) {
    ...
    fs.readdirSync(path.join(baseFolder, foldername)).forEach(function (file) {
      ...
      const options = clone(originalOptions)
  ...
  function diff (left, right) {
    require('diff').diffLines(left, right).forEach(function (item) {
      if (item.added || item.removed) {
        const text = item.value && item.value.replace('\n', String.fromCharCode(182) + '\n').replace('\ufeff', '[[BOM]]')
  ...
  function toCSS (options, filePath, callback) {
    options = options || {}
    const str = fs.readFileSync(filePath, 'utf8'); const addPath = path.dirname(filePath)
    ...
    if (options.plugin) {
      const Plugin = require(path.resolve(process.cwd(), options.plugin))
  ...
  function testNoOptions () {
    if (oneTestOnly && oneTestOnly !== 'Integration') {
      return
    }
    totalTests++
    try {
  ...
  return {
    ...
    testTypeErrors,
    testSourcemap,
    testSourcemapWithoutUrlAnnotation,
    testImports,
    testImportRedirect,
    testEmptySourcemap,
    testNoOptions,
    testDisablePluginRule,
    testJSImport,
    finished
  }
}
